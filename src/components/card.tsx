...
export function Card({ icon, title, description, ...props }: CardProps) {
  ...
  return (
    ...
    >
      {icon ? (
        <div className="not-prose mb-2 w-fit shadow-md rounded-lg border bg-fd-muted p-1.5 text-fd-muted-foreground flex items-center justify-center [&_svg]:size-4">
          {icon}
        </div>
      ) : null}
